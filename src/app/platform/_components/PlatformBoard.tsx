          ...
          <div style={{ padding: 16, display: "grid", gap: 12 }}>
            ...
            {queue.map((q, idx) => {
              ...
              const operationalLabel =
                q.label ||
                `${q.serviceName || "Servicio"} x${Math.max(1, Number(q.quantity ?? 1))} · ${Math.max(0, Number(q.pax ?? 0))} pax · ${Math.max(1, Number(q.durationMinutes ?? 0) || 1)} min`;
              return <div key={`${q.reservationId}-${q.reservationUnitId || idx}`} style={{ border: `1px solid ${queueBd}`, background: queueBg, borderRadius: 18, padding: 14, display: "grid", gap: 8, boxShadow: "0 12px 24px rgba(15, 23, 42, 0.04)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><div style={{ fontWeight: 950 }}>{q.customerName || "Sin nombre"}</div><div style={{ fontSize: 12, fontWeight: 900, color: queueFg }}>{queueWaitMs !== null ? `Espera ${msToClock(queueWaitMs)}` : operationalLabel}</div></div>
                <div style={{ fontSize: 12, opacity: 0.8 }}>{q.category ? `${q.category} | ` : ""}{operationalLabel}{q.isLicense ? " | Licencia" : ""}</div>
