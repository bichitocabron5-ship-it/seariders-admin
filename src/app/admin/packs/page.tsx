...
export default function AdminPacksPage() {
  ...
  async function addItem() {
    ...
    if (!itemOptionId) {
      setError("Selecciona una opcion valida para el item del pack.");
      return;
    }
    ...
    if (editingPackId) {
      try {
        ...
        setError(e instanceof Error ? e.message : "Error anadiendo item");
        return;
      }
    }
      ...
      <section style={{ ...softCard, padding: 20, background: "linear-gradient(135deg, #ffffff 0%, #f8fafc 45%, #eef2ff 100%)", display: "grid", gap: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "start" }}>
          <div style={{ display: "grid", gap: 6, maxWidth: 760 }}>
            ...
            <div style={{ fontSize: 14, color: "#475569" }}>Configuracion de packs, items incluidos, fuentes permitidas y vigencias comerciales.</div>
          </div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><span style={heroPill}>Packs: {packs.length}</span><span style={heroPill}>Servicios: {services.length}</span><span style={heroPill}>Opciones: {options.length}</span><span style={heroPill}>Modo: {editingPackId ? "Edicion" : "Alta"}</span></div>
      </section>

      {error ? <div style={errorBox}>{error}</div> : null}

      <section ref={formRef} style={{ ...softCard, padding: 16, display: "grid", gap: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center" }}><div style={{ fontWeight: 950, fontSize: 20 }}>{editingPackId ? "Editar pack" : "Crear pack"}</div>{editingPackId ? <span style={modeBadge}>Modo edicion</span> : null}</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10 }}><input value={code} onChange={(e) => setCode(e.target.value)} placeholder="PACK_JETSKI_FAMILY" style={inputStyle} disabled={Boolean(editingPackId)} /><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nombre" style={inputStyle} /><input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Descripcion (opcional)" style={inputStyle} /><input value={priceEuros} onChange={(e) => setPriceEuros(e.target.value)} placeholder="EUR / persona" style={{ ...inputStyle, textAlign: "right", fontWeight: 900 }} /></div>
        ...
        <div style={{ border: "1px dashed #dbe4ea", borderRadius: 16, background: "#fcfcff", padding: 14, display: "grid", gap: 10 }}><div style={{ fontWeight: 900 }}>Items del pack</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 10 }}><select value={itemServiceId} onChange={(e) => setItemServiceId(e.target.value)} style={inputStyle}>{services.map((s) => <option key={s.id} value={s.id}>{s.category ? `${s.category} · ` : ""}{s.name}</option>)}</select><select value={itemOptionId} onChange={(e) => setItemOptionId(e.target.value)} style={inputStyle} disabled={optionsForService.length === 0}><option value="">{optionsForService.length === 0 ? "Sin opciones disponibles" : "Selecciona una opcion"}</option>{optionsForService.map((o) => <option key={o.id} value={o.id}>{o.durationMinutes} min · max {o.paxMax} pax</option>)}</select><input type="number" min={1} value={itemQty} onChange={(e) => setItemQty(Number(e.target.value || 1))} style={inputStyle} /><button type="button" onClick={() => void addItem()} style={ghostBtn} disabled={!itemServiceId || !itemOptionId}>Anadir item</button></div>{items.length > 0 ? <div style={{ display: "grid", gap: 8 }}>{items.map((it, idx) => { const svc = serviceById.get(it.serviceId); const opt = it.optionId ? optionById.get(it.optionId) : null; return <div key={`${it.serviceId}-${it.optionId ?? "none"}-${idx}`} style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center", border: "1px solid #e5edf4", borderRadius: 14, padding: 12, background: "#fff" }}><div style={{ fontSize: 13 }}><b>{svc?.name ?? it.serviceId}</b>{opt ? ` · ${opt.durationMinutes} min · max ${opt.paxMax}` : " · opcion pendiente"}{` · qty ${it.quantity}`}</div><button type="button" onClick={() => removeItem(idx)} style={ghostBtn}>Quitar</button></div>; })}</div> : <div style={{ fontSize: 12, opacity: 0.7 }}>Aun no hay items en preparacion.</div>}</div>
      </section>
