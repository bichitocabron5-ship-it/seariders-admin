...
function preparedResourceSummary(contract: ContractRenderDriver) {
  ...
  if (contract.preparedAsset) {
    return {
      ...
      name: contract.preparedAsset.name ?? "EMBARCACION",
...
<body>
  ...
  <div class="page">
    ...
    <p class="clause-item"><b>h.</b> Si el usuario hace caso omiso al instructor, éste suspenderá la actividad sin que se pueda reclamar la devolución del dinero de la actividad, o la fianza de la misma.</p>
...
function buildLicensedHtml(input: ContractRenderInput) {
  ...
  const assetLabel = isJetskiContract ? "moto de agua" : "embarcacion";
  const assetLabelPlural = isJetskiContract ? "motos de agua" : "embarcaciones";
  const assetLabelTitle = isJetskiContract ? "MOTO DE AGUA" : "EMBARCACION";

  const dispatchPersonsText =
    reservation.pax ? `MAXIMO ${reservation.pax} PERSONAS EN LA ${assetLabelTitle}` : "";
...
<body>
  ...
  <div class="page">
    ...
    <table>
      ...
      <tr>
        ...
        <td class="label-cell">NÂº documento</td>
      ...
      <tr>
        ...
        <td class="label-cell">NÂº licencia</td>
  ...
  <div class="page page-break">
    ...
    <p class="clause">b. El arrendatario se obliga a no transportar a bordo un número mayor de personas que el permitido según el certificado de seguridad de la embarcación; a utilizar la embarcación sólo para cruceros de recreo. Así mismo se obliga a no ceder, subcontratar o subarrendar total o parcialmente la embarcación.</p>
    <p class="clause">c. Queda totalmente prohibido el remolque de otras embarcaciones salvo en los casos de urgencia, así mismo sólo se dejará remolcar la embarcación alquilada en los mismos casos y siempre con cabos propios para evitar los altos costes de salvamento. El arrendatario no aceptará acuerdos ni asumirá responsabilidades sin autorización de UTE JETSKI CENTER- NOMAD NAUTIC.</p>
