// src/app/api/platform/runs/[runId]/depart/route.ts
...
export async function POST(_: Request, ctx: { params: Promise<{ runId: string }> }) {
  ...
  try {
    const out = await prisma.$transaction(async (tx) => {
      ...
      // Blindaje final antes de mandar la salida al mar
      ...
      await tx.monitorRun.update({
        where: { id: runId },
        data: { status: MonitorRunStatus.IN_SEA, startedAt: now },
        select: { id: true },
      });
      ...
      for (const a of queued) {
        ...
        const expectedEndAt = new Date(now.getTime() + a.durationMinutesSnapshot * 60_000);
        ...
        touchedReservationIds.add(a.reservationId);
      }
