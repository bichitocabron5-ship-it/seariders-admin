      ...
      if (action === "mark_ready") {
        ...
        const readyCheck = evaluateReadyForPlatform({
          ...
          items: (reservation.items ?? []).map((item) => ({
            ...
            service: item.service ? { name: item.service.name ?? null, category: item.service.category ?? null } : null,
            option: item.option ? { durationMinutes: item.option.durationMinutes ?? null } : null,
          })),
